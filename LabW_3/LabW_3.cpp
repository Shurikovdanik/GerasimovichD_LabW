...
bool input (double **a, int height)
{
    ...
    if (isAuto) // Генерация случайных чисел (в описанных пределах. До 3 знаков после запятой включительно)
    {
        ...
        for (int i = 0; i <height; i++) {  // ручной ввод с консоли 
            for (int j = 0; j < height; j++) {
            std::cin >> a[i][j];
        }
        }
...
void replace (double **a, int x1, int y1, int x2, int y2)
{
...
int search (double **a, int height) 
{
   bool isNegative = 0;
