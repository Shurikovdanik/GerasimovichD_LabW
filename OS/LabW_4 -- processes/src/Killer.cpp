...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--id" && i + 1 < argc) {
            ...
            if (killById(pid)) std::cout << "Killed process with PID " << pid << "\n";
            else std::cout << "Failed to kill PID " << pid << "\n";
        } else if (arg == "--name" && i + 1 < argc) {
            std::string name = argv[++i];
            if (killByName(name)) std::cout << "Killed processes with name " << name << "\n";
            else std::cout << "No processes with name " << name << "\n";
        }
    }
