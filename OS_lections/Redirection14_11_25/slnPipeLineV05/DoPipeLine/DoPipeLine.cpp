...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	printf("argc:%d\n",argc);
	//Verify1 argv[1]="process0<streamIn 2>streamErr|process1>streamOut"
	//Verify2 argv[1]="process0<streamIn 2>streamErr|process1>>streamOut"
	wprintf(L"argv[1]=:%s\n",argv[1]);
	printf("argv[1]: Press the Key Enter to Run...\n");
	getchar();
	...
	*/
	...
	if(less!='<'||two!='2'||greater1!='>'||greater2!='>' )//|| greater22 != '>')
	{
	...
	SetFilePointer(hFileOut, 0, 0, FILE_END); //FILE_CURRENT; FILE_BEGIN; FILE_END
	//<0,0> relative 64-bit position from FILE_END
	//
	...
	wprintf(L"GetStdHandle(STD_INPUT_HANDLE): %d\n", (unsigned)GetStdHandle(STD_INPUT_HANDLE));
	wprintf(L"GetStdHandle(STD_OUTPUT_HANDLE): %d\n", (unsigned)GetStdHandle(STD_OUTPUT_HANDLE));
	wprintf(L"GetStdHandle(STD_ERROR_HANDLE): %d\n", (unsigned)GetStdHandle(STD_ERROR_HANDLE));
	...
	HANDLE w2H[]  = { process0_Info.hProcess ,process1_Info.hProcess };
	WaitForMultipleObjects(2, w2H,TRUE,INFINITE);
	CloseHandle(process0_Info.hProcess);
	CloseHandle(process1_Info.hProcess);

	printf("PipeLine complited its job!\n");
	//system("type Results2.txt");
	_wsystem(streamOut);
	printf("Press the Key Enter to Exit...\n");
	getchar();
	return 0;
