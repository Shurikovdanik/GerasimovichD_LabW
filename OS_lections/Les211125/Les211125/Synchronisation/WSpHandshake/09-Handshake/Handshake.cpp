...
void Dlg_OnClose(HWND hwnd) {
    if (hThreadServer) {
        DWORD dwExitCode = 0;
        if (GetExitCodeThread(hThreadServer, &dwExitCode) && dwExitCode == STILL_ACTIVE) {
            // Некорректное принудительное завершение по условию задания
            TerminateThread(hThreadServer, 2);
            // Желательно дождаться ядра, что объект потока перешел в signaled
...
void Dlg_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
   ...
   { // Submit a request to the server thread
      DWORD dwExitCode = 0;
      if (hThreadServer == NULL || !GetExitCodeThread(hThreadServer, &dwExitCode) || dwExitCode != STILL_ACTIVE)
      {
         HWND hBtnSubmit = GetDlgItem(hwnd, IDC_SUBMIT);
         if (hBtnSubmit)
            EnableWindow(hBtnSubmit, FALSE);
         // Можно также вывести пользователю уведомление (MessageBox), если требуется.
         break;
      }

      // Поток жив — выполняем исходный сценарий:
