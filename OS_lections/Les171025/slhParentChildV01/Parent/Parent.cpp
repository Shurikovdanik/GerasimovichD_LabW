...
int main(int argc, char* argv[])
{
    ...
    SECURITY_ATTRIBUTES saEvent = { sizeof(SECURITY_ATTRIBUTES),NULL,TRUE }; //bInheritHandle == TRUE;

    HANDLE hEvent=CreateEventA(
        &saEvent,             //[in, optional] LPSECURITY_ATTRIBUTES lpEventAttributes,
        FALSE,                //[in]           BOOL                  bManualReset,=TRUE, 
    ...
    DWORD lenQuery1 = strcspn(szQuery,"\0");
   ...
   bool bErrorFlag = WriteFile(
    ...
    sprintf_s(szCommandLine, 256, "%s %d %d", "Child.exe", (DWORD)hFile, (DWORD)hEvent);
    ...
    si.dwFlags = STARTF_USEPOSITION;//That is necessary to use the next two instructions!    
    si.dwX = 800;
    si.dwY = 500;
   ...
   // SetEvent(hEvent);
    ...
    fSuccess = CreateProcessA(
        ...
        TRUE,  //Child Process "inherits parent's kernel objects" (bInheritHandles==TRUE): hFile and hEvent
        dwFlags,
    ...
    std::cout << "Waiting for ending Child Process...\n";

   // WaitForSingleObject(pi.hProcess, INFINITE); //pi.hProcess is the HANDLE of Child Process
    DWORD dwWaitResult = WaitForSingleObject(pi.hProcess, 10000); // It is to wait for 10 seconds.

    switch (dwWaitResult) {
    case WAIT_OBJECT_0:
        std::cout << "The Child Process was signaled!" << std::endl;
        break;
    case WAIT_TIMEOUT:
        std::cout << "Timeout elapsed, The Child Process not signaled." << std::endl;
...
//}
//Key Notes
//Ensure the handle(hHandle) is valid and properly initialized.
//Always close the handle using CloseHandle() when it is no longer needed to avoid resource leaks.
//Use INFINITE cautiously to avoid deadlocks in your application.
//This function is commonly used in multithreaded applications to synchronize threads or processes.
    DWORD exitCodeChild = 0;
    GetExitCodeProcess(pi.hProcess, &exitCodeChild);

    //////////////////////////////////////////////////////////////////////////
    printf("\nChild Process with PID:%d Ended,exitCodeChild:%d...\n", pi.dwProcessId, exitCodeChild);
